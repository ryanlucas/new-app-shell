...
| `spend-only` | Spend Management only (standalone) | Standalone Spend customer — no HR/Payroll. Spend pillar plus banking and integrations. | `finance`, `custom-apps`, `platform` | — |
| `it-only` | IT only (standalone) | Standalone IT customer — devices, identity, and integrations only. | `it`, `custom-apps`, `platform` | — |
