...
| `spend-only` | Spend Management only (standalone) | Standalone Spend customer — no HR/Payroll. Spend pillar plus banking and integrations. | `spend`, `custom-apps`, `banking`, `settings` | — |
| `it-only` | IT only (standalone) | Standalone IT customer — devices, identity, and integrations only. | `it`, `identity`, `custom-apps`, `settings` | — |
